...
library(readxl)
final <- read_excel("final_proportion_data20240428.xlsx")
library(Kendall)
library(xts)
library(collapse)
library(modifiedmk)
...
ggplot(final, aes(x = year, group = 1)) +
  ...
  labs(x = "Year",
       y = "Proportion of participants indicating 'very worried'")
...
ggplot(final, aes(x = year, group = 1)) +
  ...
  geom_errorbar(aes(ymin = `gap_hate_CI_lo`, ymax = `gap_hate_CI_up`), colour = "black", width = .1) +
